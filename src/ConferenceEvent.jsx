...
const ConferenceEvent = () => {
  ...
  const venueItems = useSelector((state) => state.venue); // Access venue items from Redux store
  const avItems = useSelector((state) => state.av); // Access AV items from Redux store
  const mealsItems = useSelector((state) => state.meals); // Access meals items from Redux store
  ...
  const remainingAuditoriumQuantity =
    ...
    venueItems.find((item) => item.name === "Auditorium Hall (Capacity:200)")
      .quantity; // Calculate remaining quantity for Auditorium Hall
  ...
  const getItemsFromTotalCost = () => {
    ...
    avItems.forEach((item) => {
      if (
        item.quantity > 0 &&
        !items.some((i) => i.name === item.name && i.type === "av") // Avoid duplicates
      ) {
  ...
  const ItemsDisplay = ({ items }) => {
    console.log("ItemsDisplay items:", items);
    return (
      <>
        <div className="display_box1">
          {items.length === 0 && <h3>No items selected.</h3>}
